    ...
    : AbstractPlayer(parent), m_cookiesFile(std::move(cookiesFile_))
{
    ...
    const char* vlc_args[] = {
        ...
        "--ipv4-timeout=5000", "--network-caching=1000",  "--http-reconnect",
        "--aout=directsound",  "--plugin-path=./plugins", "--lua-intf=luaintf"};
    m_instance = libvlc_new(sizeof(vlc_args) / sizeof(vlc_args[0]), vlc_args);
    if (!m_instance)
    {
        qWarning()
            << "[YTPlayer] Failed to create libVLC instance. LibVLC error:"
            << libvlc_errmsg();
    ...
    // Optional: Attach event manager for playback events (e.g., end reached,
    // error)
    libvlc_event_manager_t* event_manager =
        libvlc_media_player_event_manager(m_player);
    libvlc_event_attach(event_manager, libvlc_MediaPlayerEndReached,
                        onMediaEndReached, this);
    ...
    // signals for yt-dlp
    connect(ytdlpProcess, &QProcess::readyReadStandardOutput, this,
            &YTPlayer::onYtdlpReadyRead);
...
{
    Q_UNUSED(event);
    auto* player = static_cast<YTPlayer*>(user_data);
...
void YTPlayer::play(const QString& url)
{
    ...
    if (rx.match(normalized).hasMatch())
        normalized = QStringLiteral("https://www.youtube.com/watch?v=%1")
                         .arg(normalized);

    pendingNormalizedUrl = normalized;
    pendingAllowPlaylist =
        normalized.contains("list=") || normalized.contains("playlist");
    ...
    QFile localYt("thirdparty/libmpv/yt-dlp.exe");  // Keep path as is, or
                                                    // adjust if needed
    QString prog =
        localYt.exists() ? localYt.fileName() : QStringLiteral("yt-dlp");
    ytdlpProcess->setProgram(prog);

    // Build args: get direct audio URL (prefer m4a)
    QStringList args;
    args << QStringLiteral("-g") << QStringLiteral("-f")
         << QStringLiteral("bestaudio")
         << QStringLiteral(
                "--no-check-certificate");  // optional, helps some environments
    ...
    {
        qWarning() << "[YTPlayer] yt-dlp failed to start. Error:"
                   << ytdlpProcess->errorString();
        emit errorOccurred("yt-dlp failed to start: " +
                           ytdlpProcess->errorString());
        return;
    }
    qDebug() << "[YTPlayer] yt-dlp started successfully. PID:"
             << ytdlpProcess->processId();
...
{
    static const QStringList supported = {"youtube", "cookies", "quitAndWait",
                                          "playlist"};
...
void YTPlayer::onYtdlpReadyReadError() const
{
    if (!ytdlpProcess) return;
    QString err =
        QString::fromUtf8(ytdlpProcess->readAllStandardError()).trimmed();
    if (!err.isEmpty())
    {
        qWarning() << "[YTPlayer] yt-dlp stderr (real-time):" << err;
        writeLog("yt_err_realtime.txt",
...
void YTPlayer::onYtdlpFinished(int exitCode, QProcess::ExitStatus)
{
    ...
    if (needFallback && !ytdlpTriedManifest)
    {
        ...
        {
            qWarning()
                << "[YTPlayer] yt-dlp failed to start for fallback manifest";
    ...
    if (stdoutStr.isEmpty())
    {
        qWarning() << "[YTPlayer] yt-dlp returned empty stdout (after "
                   ...
                   << stderrStr;
        emit errorOccurred(
            "yt-dlp returned no URL (even after fallback). See logs.");
        return;
    }
    ...
    // Создаем новый media
    m_currentMedia =
        libvlc_media_new_location(m_instance, directUrl.toUtf8().constData());
    if (!m_currentMedia)
    {
    ...
    QString headerList =
        QString("Referer: %1,User-Agent: %2").arg(refererHeader, userAgent);
    libvlc_media_add_option(
        m_currentMedia,
        (":http-header-fields=" + headerList).toUtf8().constData());

    if (!m_cookiesFile.isEmpty())
    {
        libvlc_media_add_option(
            m_currentMedia,
            (":http-cookies-file=" + m_cookiesFile).toUtf8().constData());
    }
    ...
    // НЕ ВЫЗЫВАЕМ libvlc_media_release здесь! Храним m_currentMedia для
