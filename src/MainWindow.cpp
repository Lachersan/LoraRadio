...
// Найти локальный индекс (внутри типа) для
// глобального индекса. Возвращает -1 если
// глобальный индекс неверный.
int localIndexFromGlobal(const StationManager* mgr, int globalIdx)
{
    if (!mgr) return -1;
    const auto& all = mgr->stations();
    ...
    : QMainWindow(parent), m_stations(stations), m_player(player)
{
    ...
    // Загружаем список (в очередь, чтобы не
...
void MainWindow::playStation(int idx)
{
    ...
    {
        qWarning() << "[MainWindow] playStation: "
                      "invalid index"
                   << idx;
        return;
    }
    ...
    // Устанавливаем громкость после обновления
    // индекса
    m_player->setVolume(st.volume);
    qDebug() << "[MainWindow] Setting station "
                "volume for"
             << st.url << ":" << st.volume;

    // Сохраняем *локальный* индекс в настройках
    // для данного типа
...
void MainWindow::onPlaybackStateChanged(bool isPlaying) const
{
    if (radioPage) radioPage->setPlaybackState(isPlaying);

    if (ytPage) ytPage->setPlaybackState(isPlaying);
}

void MainWindow::onPlayClicked() const
{
...
void MainWindow::onPrevClicked()
{
    ...
    if (local > 0)
    {
        ...
        if (global >= 0)
        {
            ...
            {
                ytPage->setCurrentStation(local);  // Assuming similar
                                                   // slot added to
                                                   // YouTubePage
            }
        }
    ...
    {
        qWarning() << "[Prev] Cannot go before "
                      "first station of type"
                   << type;
    }
}
...
void MainWindow::onNextClicked()
{
    ...
    {
        qWarning() << "[Next] Cannot go past "
                      "last station of type"
                   << type;
    }
}
...
void MainWindow::onVolumeChanged(int value) const
{
    m_volumeSlider->setValue(value);
    m_volumeSpin->setValue(value);
}

void MainWindow::onPlayerVolumeChanged(int value) const
{
...
void MainWindow::onVolumeMuteClicked() const
{
...
void MainWindow::onRemoveClicked(int idx)
{
    ...
    if (idx < 0 || idx >= list.size())
    {
        qWarning() << "[MainWindow] onRemoveClicked: "
                      "invalid index"
                   << idx;
        return;
    }
...
void MainWindow::onUpdateClicked(int idx)
{
    ...
    if (idx < 0 || idx >= list.size())
    {
        qWarning() << "[MainWindow] onUpdateClicked: "
                      "invalid index"
                   << idx;
        return;
    }
...
void MainWindow::onModeChanged(int newIndex)
{
    ...
    // Останавливаем плеер на странице, с которой
    // уходим
...
void MainWindow::switchLanguage(const QAction* action)
{
    ...
    QMessageBox::information(this, tr("Язык изменен"),
                             tr("Перезапустите приложения для смены "
                                "языка"));
}
...
void MainWindow::onYouTubePlayRequested(const QString& url) const
{
    qWarning() << "[MainWindow] YouTube play requested:" << url;

    if (url.isEmpty())
    {
        qWarning() << "[MainWindow] Invalid "
                      "YouTube URL!";
        return;
    }

    // Останавливаем всё, что сейчас играет (радио
    // или ютуб)
    m_player->stop();
